...
namespace ChessChallenge.Application
{
    public static class MenuUI
    {
        ...
        public static string getBotName(Type botClass) {
            return botClass.BaseType.GenericTypeArguments[0].Name;
        }
        public static void DrawButtons(ChallengeController controller)
        {
            ...
            if (NextButtonInRow(botName + " vs " + botName, ref buttonPos, spacing, buttonSize))
            {
                controller.StartNewBotMatch(ChallengeController.PlayerType.MyBot, ChallengeController.PlayerType.MyBot);
            }
            if (NextButtonInRow(botName + " vs " + evilBotName, ref buttonPos, spacing, buttonSize))
            {
