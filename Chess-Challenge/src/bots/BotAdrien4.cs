...
// - Although overall play seems more solid, due to the blunders done by the AI and its incapability to make a plan late game
...
namespace BotAdrien4{
    ...
    public class BotAdrien4 : IChessBot
    {
        ...
        private const float BLUNDER_MOVE_SCORE = -5, TOO_GOOD_TO_WORK_MOVE_SCORE = 10, CHECKMATE_SCORE = 10000, PROTECT_NON_THREATENED_SCORE_WEIGHT=.2f;
        ...
        private float ResolveFight(PieceType piece, List<PieceType> protectors, List<PieceType> attackers) {
            ...
            if (nbProtectors >= nbAttackers)
            {
                ...
                remainingProtectionScore = (nbProtectors - nbAttackers) * protectorScores.ElementAt(nbAttackers) * PROTECT_NON_THREATENED_SCORE_WEIGHT;
            } else {
